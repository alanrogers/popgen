...
class hapmap_dataset:
    ...
    def  __init__(self, infile, dirpath=hapmap_dir):
        if dirpath==None:
            dirpath = get_dirpath()
        self.filename = os.path.join(dirpath, infile)
        try:
            if infile.endswith(".gz"):
                linelist = gzip.open(self.filename, "rt")
            else:
                linelist = open(self.filename, "r")
        except:
            raise IOError("hapmap_dataset.__init__:" +\
...
def hapmap_fname(chromosome, pop, dirpath=hapmap_dir):
    ...
    """
    pop = pop.upper()
    pat = "^genotypes_chr%d_%s_.*_fwd\.txt(\.gz)?$" % (chromosome, pop)
