...
\begin{frame}
\frametitle{Normalizing $D^2$}
Square $D$ to remove sign and normalize by heterozygosity:
\begin{eqnarray*} 
r^2 &=& \frac{D^2}{a(1-a)b(1-b)}\qquad\hbox{\small (Hill \& Robertson, 1968)}\\
&=& \frac{D^2}{H_A H_B/4}, 
\end{eqnarray*}
where $H_A = 2a(1-a)$ is heterozygosity at locus $A$, and
$H_B = 2b(1-b)$ is heterozygosity at $B$.
\end{frame}
...
\]
\begin{itemize}
\item Estimate expectations by averaging across genome.
\item Deterministic theory available.
\item Often used as an approximation to $E[r^2]$.
\item But it's a lousy approximation.
\end{itemize}
\column{0.5\textwidth}
\pause
\[
 \sigma_d^2 = \frac{E[H_A H_B r^2]}{E[H_A H_B]}
\]
\begin{itemize}
\item $\sigma_d^2$ is $r^2$ weighted by heterozygosities.
\item Insensitive to sequencing error.
\item Weighting by heterozygosity emphasizes loci with deep gene
  trees. 
\item $\sigma_d^2$ sensitive to ancient history.
\end{itemize}
\end{columns}
\end{frame}

\begin{frame}
\frametitle{Loci with high heterozygosity have deep gene trees}
{\centering\input{figTH}\\}
\end{frame}

\begin{frame}
\frametitle{European LD curve for chromosome 1}
{\centering\input{figceuraw}\\}
\end{frame}
...
{\centering\input{figldgro}\\}
\end{frame}

\begin{frame}
\frametitle{Population decline}
{\centering\input{figshrinkmodel}\\}
...
{\centering\input{figshrink}\\}
\end{frame}

\begin{frame}
\frametitle{A ``bottleneck''}
...
{\centering\input{figceu01}\\}
\end{frame}

\begin{frame}
\frametitle{3-parameter model of history}
{\centering\input{figsudden}\\}
...
\begin{frame}
\frametitle{Estimates from simulation of expanded population}
{\centering\input{figgrotp}\\}
Shows same estimates as time path.
\end{frame}

\begin{frame}
\frametitle{Estimates from European chromosomes {\tiny(1000-Genomes Prj)}}
{\centering\input{figceutp1}\\}
\end{frame}

\begin{frame}
\frametitle{Estimates from European chromosomes {\tiny(1000-Genomes Prj)}}
{\centering\input{figceutp2}\\}
\end{frame}

\begin{frame}
\frametitle{Estimates from European chromosomes {\tiny(1000-Genomes Prj)}}
{\centering\input{figceutp3}\\}
\end{frame}

\begin{frame}
\frametitle{Estimates from European chromosomes {\tiny(1000-Genomes Prj)}}
{\centering\input{figceutp4}\\}
\end{frame}
